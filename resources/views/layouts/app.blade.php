...
<head>
    ...
    <meta name="description"
        content="{{ setting('site_meta_description', 'موبايلات المواصي - وجهتك الموثوقة لأحدث الهواتف الذكية وملحقاتها في غزة') }}">
    <meta name="keywords" content="{{ setting('site_keywords', 'موبايلات, هواتف ذكية, غزة, فلسطين') }}">
    @if (setting('site_favicon'))
        <link rel="icon" type="image/x-icon" href="{{ asset('storage/' . setting('site_favicon')) }}">
    @endif
    <title>@yield('title', setting('site_meta_title', 'موبايلات المواصي - غزة'))</title>
        ...
        <div class="container mx-auto px-4 flex flex-wrap justify-between items-center gap-2">
            <div class="flex items-center space-x-2 sm:space-x-4 space-x-reverse">
                @if (setting('site_phone'))
                    <a href="tel:{{ setting('site_phone') }}" class="hover:text-white">
                        <i class="fas fa-phone-alt ml-1 sm:ml-2"></i>
                        <span class="hidden xs:inline">{{ setting('site_phone') }}</span>
                    </a>
                @endif
                @if (setting('site_email'))
                    <a href="mailto:{{ setting('site_email') }}" class="hidden md:inline hover:text-white">
                        <i class="fas fa-envelope ml-2"></i> {{ setting('site_email') }}
                    </a>
                @endif
            </div>
            <div class="flex items-center space-x-3 sm:space-x-4 space-x-reverse">
                @if (setting('site_location'))
                    <span class="text-yellow-400 text-xs sm:text-sm">
                        <i class="fas fa-map-marker-alt ml-1"></i>
                        <span class="hidden sm:inline">{{ setting('site_location') }}</span>
                    </span>
                @endif
                <div class="flex items-center space-x-2 space-x-reverse">
                    @if (setting('site_facebook'))
                        <a href="{{ setting('site_facebook') }}" target="_blank" class="hover:text-white p-1"><i
                                class="fab fa-facebook-f"></i></a>
                    @endif
                    @if (setting('site_instagram'))
                        <a href="{{ setting('site_instagram') }}" target="_blank" class="hover:text-white p-1"><i
                                class="fab fa-instagram"></i></a>
                    @endif
                    @if (setting('site_whatsapp'))
                        <a href="https://wa.me/{{ setting('site_whatsapp') }}" target="_blank"
    ...
    <header class="bg-white shadow-md sticky top-0 z-50">
        <div class="container mx-auto px-4">
            <div class="flex items-center justify-between py-3 sm:py-4">
                <a href="{{ route('home') }}" class="flex items-center space-x-2 space-x-reverse flex-shrink-0">
                    ...
                    <div class="flex flex-col">
                        <span
                            class="text-base sm:text-xl font-bold text-gray-800">{{ setting('site_name', 'موبايلات المواصي') }}</span>
                        @if (setting('site_location'))
                            <span class="text-xs text-gray-500 hidden sm:inline">{{ setting('site_location') }}</span>
                        @endif
                    </div>
                </a>
    ...
    <footer class="bg-gray-900 text-gray-300 mt-8 sm:mt-12">
        <div class="container mx-auto px-4 py-8 sm:py-12">
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-8">
                <div class="sm:col-span-2 lg:col-span-1">
                    <div class="flex items-center space-x-2 space-x-reverse mb-4">
                        @if (setting('site_footer_logo'))
                            <img src="{{ asset('storage/' . setting('site_footer_logo')) }}"
                                alt="{{ setting('site_name') }}" class="h-10 sm:h-12 w-auto">
                        @else
                            <div
                        ...
                        @endif
                        <span
                            class="text-lg sm:text-xl font-bold text-white">{{ setting('site_name', 'موبايلات المواصي') }}</span>
                    </div>
                    <p class="text-sm leading-relaxed">
                        {{ setting('site_description', 'وجهتك الموثوقة لأحدث الهواتف الذكية وملحقاتها في غزة. منتجات أصلية بأسعار منافسة وخدمة ما بعد البيع ممتازة.') }}
                    </p>
                    @if (setting('site_address'))
                        <div class="mt-4 bg-gray-800 rounded-lg p-3">
                            <p class="text-yellow-400 text-sm font-semibold">
                                ...
                                {{ setting('site_location', 'غزة') }}
                            </p>
                            <p class="text-gray-400 text-xs mt-1">{{ setting('site_address') }}</p>
                        </div>
                    @endif

                    @if (setting('site_facebook') || setting('site_instagram') || setting('site_whatsapp'))
                ...
                <div>
                    ...
                    <ul class="space-y-2 text-sm">
                        ...
                        <li><a href="{{ route('products.index') }}" class="hover:text-white transition">جميع
                                المنتجات</a></li>
                        <li><a href="{{ route('brands.index') }}" class="hover:text-white transition">العلامات
                                التجارية</a></li>
                ...
                <div>
                    ...
                    <ul class="space-y-2 text-sm">
                        <li><a href="{{ route('products.index', ['category' => 'flagship']) }}"
                                class="hover:text-white transition">الهواتف الرائدة</a></li>
                        <li><a href="{{ route('products.index', ['category' => 'mid-range']) }}"
                                class="hover:text-white transition">الفئة المتوسطة</a></li>
                        <li><a href="{{ route('products.index', ['category' => 'budget']) }}"
                    ...
                    <ul class="space-y-3 text-sm">
                        ...
                        @if (setting('site_phone'))
                            <li class="flex items-center">
                                ...
                                <a href="tel:{{ setting('site_phone') }}" dir="ltr"
                                    class="hover:text-white transition">{{ setting('site_phone') }}</a>
                            </li>
                        @endif
                        @if (setting('site_email'))
                            <li class="flex items-center">
                                ...
                                <a href="mailto:{{ setting('site_email') }}"
                                    class="hover:text-white transition break-all">{{ setting('site_email') }}</a>
                            </li>
                        @endif
                    </ul>
                </div>
            ...
            <div class="border-t border-gray-800 mt-6 sm:mt-8 pt-6 sm:pt-8 text-center text-xs sm:text-sm">
                <p>&copy; {{ date('Y') }} {{ setting('site_name', 'موبايلات المواصي') }}. جميع الحقوق محفوظة.</p>
